...
import { metaIntegrations, whatsappConversations, whatsappMessages } from '../db/schema'
...
export class WhatsappService {
  ...
  private readonly decryptFailures = new Map<string, { count: number; lastAt: number }>()
  ...
  private registerDecryptFailure(accountId: string) {
    ...
    if (nextCount >= 5 && !this.resetInFlight.has(accountId)) {
      ...
      this.emitEvent(accountId, { type: 'qr_required', reason: 'decrypt_failed' })
  ...
  private async persistWppMessage(accountId: string, message: any) {
    ...
    const contactJid = target.includes('@') ? target : `${this.normalizeContact(target)}@c.us`
    ...
    const isBase64ImageBody = (val: string) => {
      ...
      if (trimmed.length > 200 && /^[A-Za-z0-9+/=]+$/.test(trimmed)) return true
      return false
    }
    const caption =
      typeof message?.caption === 'string' && message.caption.trim() ? message.caption.trim() : ''
    const replyToWamid =
      typeof message?.quotedMsgId === 'string'
        ? message.quotedMsgId
        : message?.quotedMsg?._serialized || message?.quotedMsg?.id || null
    let body =
      typeof message?.body === 'string' && message.body.trim() ? message.body.trim() : ''
    ...
    const contactName =
      direction === 'INBOUND'
        ? message?.sender?.pushname || message?.sender?.name || message?.notifyName || null
        : null
    const status =
      direction === 'INBOUND' ? 'delivered' : this.mapAckToStatus(typeof message?.ack === 'number' ? message.ack : 1)
  ...
  async startQr(accountId: string, options?: { reset?: boolean }) {
    ...
    if (existing && existing.status === 'connected' && this.sockets.has(accountId)) {
      return existing
    }
        ...
        .then((client) => {
          ...
          const handleMessage = (message: any) => {
            ...
            this.persistWppMessage(accountId, message).catch((error) => {
              console.error('[wpp] persist message failed', {
                accountId,
                error,
                messageId: message?.id?._serialized ?? message?.id?.id ?? message?.id ?? null,
              })
            })
          }
          ...
          if (typeof (client as any).onAck === 'function') {
            ;(client as any).onAck((...args: any[]) => {
              ...
              const wamid =
                typeof messageId === 'string' ? messageId : this.extractWamid(messageId) || null
              if (wamid) {
                this.updateMessageStatusByWamid(accountId, String(wamid), ackNumber).catch(() => {
  ...
  private async syncRecentHistory(accountId: string, client: any) {
    ...
    try {
      if (typeof client.getAllChats !== 'function' || typeof client.getAllMessagesInChat !== 'function') {
        return
      }
              ...
              const serialized =
                typeof chat?.id?._serialized === 'string'
                  ...
                  : typeof chat?.id === 'string'
                    ? chat.id
                    : ''
              const isGroup = Boolean(chat?.isGroup) || serialized.endsWith('@g.us') || serialized.endsWith('g.us')
              return !isGroup
            })
            .slice(0, 30)
        : []
        ...
        const chatId =
          typeof chat?.id?._serialized === 'string'
            ...
            : typeof chat?.id === 'string'
              ...
              : null
        if (!chatId) continue
        const isGroup = Boolean(chat?.isGroup) || chatId.endsWith('@g.us') || chatId.endsWith('g.us')
        if (isGroup) continue
        let messages: any[] = []
        try {
  ...
  async updateMessageStatusByWamid(accountId: string, wamid: string, ack: number) {
    if (!wamid) return
  ...
  async findIntegrationByPhoneNumberId(phoneNumberId: string) {
    ...
    for (const integration of integrations) {
      try {
        const payload = decryptPayload(integration.encryptedPayload) as CloudCredentials
  ...
  async getMetaAccessToken(accountId: string) {
    ...
    try {
      const payload = decryptPayload(existing[0].encryptedPayload) as { accessToken?: string }
  ...
  async connectCloudFromMeta(accountId: string) {
    ...
    return { status: 'PENDING', phoneNumberId, displayPhone, verifyToken, webhookUrl }
  }
  ...
  async updateConversationName(accountId: string, contactPhone: string, contactName: string) {
    await db
      .update(whatsappConversations)
  ...
  async updateConversationClassification(
    ...
    await db
      .update(whatsappConversations)
      .set({
        classification: classification && classification.trim() ? classification.trim() : null,
        updatedAt: new Date(),
      })
      .where(
        and(
  ...
  async listPipeline(accountId: string) {
    ...
    const defaultStages = ['entrando', 'qualificacao', 'proposta', 'fechamento']
          ...
          mime =
            typeof payload?.mimetype === 'string'
              ...
              : type === 'sticker'
                ? 'image/webp'
                : null
          const dur = typeof payload?.duration === 'string' ? Number(payload.duration) : payload?.duration
  ...
  private async getLastMessageTimestampForPhone(accountId: string, contactPhone: string) {
  ...
  async sendOutboundMessage(
    ...
    const conversation = await this.getConversationById(accountId, conversationId)
    if (!conversation) {
    ...
    const normalizedContact = this.sanitizeContactPhone(conversation.contactPhone)
    if (!normalizedContact) {
      throw new Error('Invalid contact phone')
    }
    const fallbackJid = this.buildContactJid(normalizedContact)
    const lastNormalized = lastJid ? this.normalizeContact(String(lastJid)) : ''
    const jid =
  ...
  async getMessageMedia(accountId: string, conversationId: string, messageId: string) {
    ...
    if (!message.length) {
      ...
      const fallback = await db
        ...
        .where(and(eq(whatsappMessages.id, messageId), eq(whatsappConversations.accountId, accountId)))
        .limit(1)
      if (!fallback.length) {
    ...
    const isAudio = type === 'ptt' || type === 'audio' || mimetype.startsWith('audio')
